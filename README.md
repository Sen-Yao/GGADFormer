...
```bash
conda create -n VecGAD python=3.8 -y
conda activate VecGAD
...
为系统性地应对上述挑战，我们设计并提出了VecGAD，一个基于 Transformer 架构的生成式图异常检测框架。VecGAD 的先进性主要体現在其四大核心贡献：1) 一种解耦式的图结构感知输入编码策略，它将图拓扑信息的提取与节点表征学习分离，使得模型能够进行高效的小批量训练，天然地适用于大规模图；2) 图结构感知的输入编码策略，防止过度平滑；3) 一种新颖的生成式策略，在缺乏真实异常样本的情况下，**人工合成高质量的伪异常样本**；4) 多重学习机制，确保模型学习到鲁棒且具有区分性的节点表征。

VecGAD 的创新点包括

- 首个基于 Transformer 的生成式半监督图异常检测模型
- 支持 mini-batch 训练，对大图有良好的可扩展性
- 相比传统的半监督异常检测方法采用的 15% 的划分比，VecGAD 可以在 5% 的划分比的场景下达到非常优秀的性能
...
|VecGAD|0.9324±0.0189|0.5629±0.0161|0.8268±0.0281|0.7221±0.0441|0.9077±0.0039|0.6534±0.0195|0.5568±0.0147
...
|VecGAD|0.8080±0.0088|0.0418±0.0042|0.5216±0.0419|0.2268±0.0755|0.6589±0.0323|0.3063±0.0138|0.0375±0.0020
