...
为实现这一目标，我们设计并提出了生成式图异常检测的图 Transformer (Generative Graph Anomaly Detection Transformer, VecGAD)。该框架巧妙地利用 Transformer 自编码器的结构，在对正常节点进行表征学习的同时，捕获其结构化重构误差。随后，它将此高维误差向量跨空间投影 (cross-space projection) 至嵌入空间，将其转化为一个有意义的、指向“最可疑”异常方向的扰动向量。通过将正常节点的嵌入沿着此方向进行偏移，VecGAD 能够自适应地生成高质量的伪异常样本。这一“自我挑战”的闭环机制，迫使模型学习到一个对正常模式更紧凑、更精确的表征。
...
- 系统性地验证了模型在极端标签稀缺场景下的有效性。 我们的方法专为真实工业场景中标签极度稀缺的挑战而设计。通过在多个基准数据集上的大量实验，我们证明了 VecGAD 在训练标签比例远低于传统设定的严苛条件下，依然能够取得卓越的性能，填补了该领域在实际应用场景下的研究空白。

# 模型架构

在此节我们将介绍 VecGAD 的各个组成部分。
...
|VecGAD|0.9324±0.0189|0.5629±0.0161|0.8183±0.0202|0.7221±0.0441|0.9077±0.0039|0.6534±0.0195|0.5568±0.0147
...
|VecGAD|0.8080±0.0088|0.0418±0.0042|0.4756±0.0585|0.2268±0.0755|0.6589±0.0323|0.3063±0.0138|0.0375±0.0020
